...
import {
  continueGuidedDiagnostic,
  GuidedDiagnosticInput,
} from "@/ai/flows/guided-diagnostic-flow";
...
export function GuidedDiagnostics() {
  ...
  const onSubmit = (values: FormValues) => {
    ...
    startTransition(async () => {
      try {
        const input: GuidedDiagnosticInput = {
            history: messages,
            message: values.message,
            apiKey: apiKey,
            apiEndpoint: apiEndpoint ? apiEndpoint : undefined,
        };
        const responseText = await continueGuidedDiagnostic(input);
        setMessages([
          ...newMessages,
        ...
        <div className="space-y-6">
          {messages.length === 0 && (
            <div className="text-center text-muted-foreground pt-8">
              Bắt đầu bằng cách mô tả sự cố của bạn (ví dụ: "Xe Komatsu PC200-8 bị mất áp suất thủy lực ở gầu").
            </div>
          )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1">
                  <FormControl>
                    <Input
                      placeholder="Nhập kết quả hoặc câu hỏi tiếp theo..."
                      {...field}
                      autoComplete="off"
                      disabled={isPending}
                    />
